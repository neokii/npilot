...
class Controls:
  ...
  def state_control(self, CS):
    ...
    if not self.joystick_mode:
      ...
      actuators.accel = self.LoC.update(self.active and CS.cruiseState.enabledAcc, CS, self.CP, long_plan, pid_accel_limits, long_rcv_frames, self.sm['radarState'])

      # Steering PID loop and lateral MPC
      lat_active = self.active and not CS.steerWarning and not CS.steerError and CS.vEgo > self.CP.minSteerSpeed
      lat_rcv_frames = self.sm.frame - self.sm.rcv_frame['lateralPlan']
